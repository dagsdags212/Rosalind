...
after the n-th month, then we obtain the Fibonacci sequence having terms Fn that are defined
by the recurrence relation Fn=Fn−1+Fn−2 (with F1=F2=1 to initiate the sequence). Although the
...
The total number of rabbit pairs that will be present after n months, if we begin with 1 pair and in each
...
def fib_rabbits(n: int, k: int) -> int:
    """
    Computes for the number of rabbit pairs after `n` generations, given that each rabbit pair 
    produces `k` offspring each generation. Rabbits require a month to sexually mature.

    PARAMETRS
    =========
    > n (int): number of generations
    > k (int): number of offspring produced by mature rabbits in each generation

    RETURN
    ======
    Total number of rabbits after `n` generations
    """
    # base case
    if n <= 1:
        return n
